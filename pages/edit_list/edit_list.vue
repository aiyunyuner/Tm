...
<script>
	export default {
		...
		onShow() {
			...
			uni.request({
				url: 0 > that.mu ? 'http://www.rural.abc/sights/me' :
				'http://www.rural.abc/sights/getMore', //仅为示例，并非真实接口地址。
				method: 'POST',
				data: {
		...
		methods: {
			goSightsPages(id) {
				uni.navigateTo({
					url: `/pages/update_sight/update_sight?e=${id}`
				})
			},
			getData() {
				let that = this
